...
export const Route = createFileRoute('/_authenticated/_user/user-loyalty')({
...
function RouteComponent() {
  ...
  const getTierLevel = (name) =>
    tierHierarchy.findIndex((t) => t.toLowerCase() === name?.toLowerCase()) + 1;
  ...
  const rfmChartData = [
    { label: 'Recency', value: recencyScore, raw: `${recency} days ago`, color: '#a855f7' },
    {
      ...
      raw: `${frequency} purchase${frequency !== 1 ? 's' : ''}`,
      color: '#f97316',
    },
    {
      label: 'Monetary',
      value: monetaryScore,
      raw: `LKR ${monetary.toFixed(2)}`,
      color: '#22c55e',
    },
  ];
            ...
            <Card key={index} className="overflow-hidden">
              <CardContent className="space-y-2">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-xs uppercase tracking-[0.3em] text-gray-400">{card.title}</p>
                    <p className="mt-2 text-2xl font-semibold text-gray-900">{card.value}</p>
                  </div>
                  <div className="rounded-2xl p-2" style={{ background: card.bg }}>
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="space-y-4">
            ...
            <div
              ...
              style={{
                background: `linear-gradient(135deg, ${tierColor}cc 0%, ${tierColor} 100%)`,
              }}
            >
                ...
                <div className="flex flex-wrap gap-2">
                  {preferredCategories.map((cat, i) => (
                    <span
                      ...
                      style={{
                        background: `linear-gradient(135deg, ${tierColor}bb, ${tierColor})`,
                      }}
                    >
                      {cat}
                    </span>
            ...
            <CardContent className="pt-5 pb-5">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-2xl font-black" style={{ color: segmentColor }}>
                      {segment}
                    </span>
                  </div>
                  {behavior && <p className="text-sm text-gray-500 mt-1 max-w-xs">{behavior}</p>}
                </div>
                <div className="p-3 rounded-2xl" style={{ background: `${segmentColor}18` }}>
              ...
              <div className="grid grid-cols-3 gap-3 mt-4">
                {[
                  ...
                  {
                    label: 'Monetary',
                    raw: `LKR ${monetary.toFixed(0)}`,
                    desc: 'Avg order value',
                    color: '#22c55e',
                  },
                ].map((m, i) => (
                  <div
                    ...
                    content={({ active, payload }) => {
                      ...
                      return (
                        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 shadow-lg text-sm">
                          ...
                          <p className="font-bold mt-1" style={{ color: d.color }}>
                            Score: {d.value}
                          </p>
                        </div>
                      );
