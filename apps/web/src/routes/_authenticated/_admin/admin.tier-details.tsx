...
function buildCategoryContributions(customers: BackendCustomerProfile[]) {
  ...
  customers.forEach((c) => {
    ...
    Object.entries(c.category_preference.category_contributions).forEach(([cat, ratio]) => {
...
function RouteComponent() {
  ...
  async function loadTier(tierKey: string) {
    try {
      ...
      const tierCustomers = data.filter((c) => c.rfm.parent_cluster.behavior === tierKey);
      ...
      tierCustomers.forEach((c) => {
        ...
        totalScore += behavioralScore(c.rfm.metrics.recency, c.rfm.metrics.frequency);
  ...
  if (!tier) {
    return <div className="space-y-6 text-center text-gray-500 py-12">Loading tier details...</div>;
  }

  const behavioralProfile = [
    {
      metric: 'Recency',
      value: 100 - customers.reduce((s, c) => s + c.rfm.metrics.recency, 0) / customers.length,
    },
    {
      metric: 'Frequency',
      value: (customers.reduce((s, c) => s + c.rfm.metrics.frequency, 0) / customers.length) * 20,
    },
    {
      metric: 'Monetary',
      value: customers.reduce((s, c) => s + c.rfm.metrics.monetary, 0) / customers.length / 50,
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          value={tier.avg_behavioral_score.toFixed(1)}
        />
        <StatCard
          icon={AlertTriangle}
          label="Churn Risk"
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2 text-sm">
            {tier.behavioral_traits.map((segment) => (
              <span key={segment} className="px-3 py-1 bg-gray-100 dark:bg-gray-700 rounded-full">
      ...
      {hasCategoryData && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <StatCard icon={Tag} label="Top Category" value={topCategory} />
            <StatCard
              icon={DollarSign}
