...
function RouteComponent() {
  ...
  const filteredCustomers = customers.filter((c) => {
    const segmentMatch = segmentFilter === 'all' || c.sub_cluster?.segment === segmentFilter;

    const searchMatch = search === '' || String(c.customer_id).includes(search);

    const frequencyMatch = minFrequency === '' || c.metrics.frequency >= Number(minFrequency);

    const monetaryMatch = minMonetary === '' || c.metrics.monetary >= Number(minMonetary);
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filteredCustomers.map((customer) => (
                    ...
                    >
                      <td className="p-3 font-semibold">#{customer.customer_id}</td>
