...
-- \copy catalog.product_import_staging(
...
WITH normalized_staging AS (
  SELECT DISTINCT
    ...
    NULLIF(regexp_replace(trim(coalesce(price, '')), '[^0-9\.-]', '', 'g'), '') AS price_text,
    NULLIF(trim(product_name_si), '') AS product_name_si
  FROM catalog.product_import_staging
...
SELECT
  COALESCE(product_name_en, product_name_si, 'Imported Product ' || product_id) AS name,
  NULL AS description,
  ...
  NOW() AS created_at,
  NOW() AS updated_at
FROM normalized_staging
ON CONFLICT (sku) DO UPDATE
SET
  name = COALESCE(NULLIF(EXCLUDED.name, ''), public.products.name),
...
FROM catalog.product_import_staging s
...
-- 4) Resolve each staging row to a canonical product id used by catalog tables.
...
INSERT INTO catalog.product_localization (
  product_id,
  locale,
  product_name,
  category,
...
INSERT INTO catalog.product_alias (product_id, locale, alias)
SELECT product_id, 'si', alias
FROM (
  SELECT resolved_product_id AS product_id, product_name_si AS alias
  FROM resolved_import
  UNION ALL
  SELECT resolved_product_id AS product_id, base_product_si AS alias
  FROM resolved_import
) t
...
-- TRUNCATE TABLE catalog.product_import_staging;
