...
RUN pnpm nx build web \
    ...
    && if [ -d dist/apps/web ]; then \
      cp -R dist/apps/web/. /tmp/web-dist/; \
    elif [ -d apps/web/dist ]; then \
      cp -R apps/web/dist/. /tmp/web-dist/; \
    else \
      find . -maxdepth 4 -type f -name index.html -path '*dist*' -print; \
      exit 1; \
    fi
